...
However, when the rules above are used to denote matrix multiplication ($ik, kj \rightarrow ij$, as depicted below), the order of a matrix appears to change. 

![](../images/16_einstein_summation/1.png)

The first matrix is $i \times k$. However, $k$ appears to denote the rows meaning we end up with a column by row matrix. And in the second matrix, $k$ appears to denote the columns instead of rows. 
