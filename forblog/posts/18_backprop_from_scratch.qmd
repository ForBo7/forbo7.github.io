...
This guide assumes a basic understanding of derivatives.
:::
...
Let's simplify things by introducing a bunch of intermediate variables.
...
$$
\frac{\partial}{\partial \vec{w}_1} \text{MSE} = \frac{\partial}{\partial \vec{w}_1} \frac{1}{N} \sum^N_{i=1} u_5 = \frac{1}{N} \sum^N_{i=1} \frac{\partial u^5}{\partial \vec{w}_1} = \frac{1}{N} \sum^N_{i=1} \frac{\partial u_5}{\partial u_4} \cdot \frac{\partial u_4}{\partial u_3} \cdot \frac{\partial u_3}{\partial u_2} \cdot \frac{\partial u_2}{\partial u_1} \cdot \frac{\partial u_1}{\partial \vec{w}_1}
$$
...
Now we can tackle finding the gradients for $w_1$. To do so, let's find the gradients of each intermediate variable.
...
$$
\frac{\partial \text{MSE}}{\partial \vec{\rm{w}}_1} = \frac{\partial u_5}{\partial \vec{\rm{w}}_1} = \frac{1}{N} \sum^N_{i=1} (2u_4) \cdot (-1) \cdot \left(\vec{\rm{w}}^T_2\right) \cdot \left(\begin{cases} 0 & \vec{\rm{u}}_1 ≤ 0 \\ 1 & \vec{\rm{u}}_1 > 0 \end{cases}\right) \cdot \left(\vec{\rm{x}}^T_i\right)
$$

And it all eventually simplifies out to the following.
...
And there you go! We've derived the formula that will allow us to calculate the gradients of $w_1$.
...
:::{.callout-important appearance="simple"}
The shapes may differ according to the data, functions, and classes you are working with, so be sure to change the shapes accordingly.
:::
...
\begin{align}
  \frac{\partial \text{MSE}}{\partial b_1} &=
    \begin{cases}
      0 & \vec{\rm{x}}_i \cdot \vec{\rm{w}}_1 + b_1 ≤ 0 \\
      \frac{2}{N} \sum^N_{i=1} (\vec{\rm{y}}_i - \text{max}(0, \vec{\rm{x}}_i \cdot \vec{\rm{w}}_1 + b_1) \cdot \vec{\rm{w}}_2 + b_2) \cdot \vec{\rm{w}}_2^T & \vec{\rm{x}}_i \cdot \vec{\rm{w}}_1 + b_1 > 0
    \end{cases} \\
  \frac{\partial \text{MSE}}{\partial \vec{\rm{x}}_i} &=
    ...
    \end{cases} \\
  \frac{\partial \text{MSE}}{\partial \vec{\rm{w}}_2} &= \frac{2}{N} \sum^N_{i=1} (\vec{\rm{y}}_i - \text{max}(0, \vec{\rm{x}}_i \cdot \vec{\rm{w}}_1 + b_1) \cdot \vec{\rm{w}}_2 + b_2) \cdot \text{max}(0, \vec{\rm{x}}_i \cdot \vec{\rm{w}}_1 + b_1) \\
  \frac{\partial \text{MSE}}{\partial b_2} &= \frac{2}{N} \sum^N_{i=1} \vec{\rm{y}}_i - \text{max}(0, \vec{\rm{x}}_i \cdot \vec{\rm{w}}_1 + b_1) \cdot \vec{\rm{w}}_2 + b_2
\end{align}
$$
:::
