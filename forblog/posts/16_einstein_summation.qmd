...
The first matrix is $i \times j$. However, $j$ appears to denote the rows meaning we end up with a column by row matrix. But even if we let $j$ denote the columns, we end up doing dot products with each _column_ in the first matrix and with each _row_ in the second matrix.
